...
const EditProfile = ({ profile: { profile, loading }, createProfile, getCurrentProfile, history }) => {
    ...
    const onChange = e => {
        let copy = cloneDeep(formData)
        const newFormData = set(copy, e.target.name, e.target.value)
        setFormData({
            ...newFormData,
            edit: true
        })
            ...
            <form className="form" onSubmit={onSubmit}>
                ...
                {displaySocialInputs && <Fragment>
                    <div className="form-group social-input">
                        ...
                        <input type="text" placeholder="Twitter URL" name="social.twitter" value={twitter}
                               onChange={onChange}/>
                    </div>

                    <div className="form-group social-input">
                        <i className="fab fa-facebook fa-2x"></i>
                        <input type="text" placeholder="Facebook URL" name="social.facebook" value={facebook}
                               onChange={onChange}/>
                    </div>

                    <div className="form-group social-input">
                        <i className="fab fa-youtube fa-2x"></i>
                        <input type="text" placeholder="YouTube URL" name="social.youtube" value={youtube}
                               onChange={onChange}/>
                    </div>

                    <div className="form-group social-input">
                        <i className="fab fa-linkedin fa-2x"></i>
                        <input type="text" placeholder="Linkedin URL" name="social.linkedin" value={linkedin}
                               onChange={onChange}/>
                    </div>

                    <div className="form-group social-input">
                        <i className="fab fa-instagram fa-2x"></i>
                        <input type="text" placeholder="Instagram URL" name="social.instagram" value={instagram}
                               onChange={onChange}/>
                    </div>
                </Fragment>}
